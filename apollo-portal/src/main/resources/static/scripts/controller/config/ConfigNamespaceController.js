application_module.controller("ConfigNamespaceController",
                              ['$rootScope', '$scope', 'toastr', 'AppUtil', 'EventManager', 'ConfigService',
                               'PermissionService', 'ReleaseService', 'UserService', 'NamespaceBranchService', 'NamespaceService',
                               controller]);
...
function controller($rootScope, $scope, toastr, AppUtil, EventManager, ConfigService,
    ...
    $scope.selectConfig = function(namespace) {
        $scope.items = namespace.viewItems;
        AppUtil.showModal('#testModal');
    };

    $scope.testDbConnection = function(driver, url, username, password) {
        ReleaseService.testDbConnection(driver, url, username, password).then(
            function (result) {
                toastr.success("连接成功");
            },function (result) {
                toastr.error(AppUtil.errorMsg(result), "连接失败");
            }
        )
    };

    $scope.testCacheConnection = function(port, host, password) {
        ReleaseService.testCacheConnection(port, host, password).then(
            function (result) {
                toastr.success("连接成功");
            },function (result) {
                toastr.error(AppUtil.errorMsg(result), "连接失败");
    ...
    $scope.tableViewOperType = '', $scope.item = {}, $scope.items={};
