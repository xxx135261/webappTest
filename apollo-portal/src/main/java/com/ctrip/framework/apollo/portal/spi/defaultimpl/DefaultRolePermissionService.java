...
public class DefaultRolePermissionService implements RolePermissionService {
    ...
    public Role createRoleWithPermissions(Role role, Set<Long> permissionIds) {
        ...
        createRolePermissions(createdRole, permissionIds);

        return createdRole;
    }

    @Transactional
    public void createRolePermissions(final Role role, final Set<Long> permissionIds) {
        Preconditions.checkState(findRoleByRoleName(role.getRoleName()) != null, "Role %s doesn't exist!", role.getRoleName());
        ...
        if (!CollectionUtils.isEmpty(permissionIds)) {
            Iterable<RolePermission> rolePermissions = FluentIterable.from(permissionIds).transform(
                permissionId -> {
                    ...
                    rolePermission.setDataChangeCreatedBy(role.getDataChangeCreatedBy());
                    rolePermission.setDataChangeLastModifiedBy(role.getDataChangeLastModifiedBy());
    ...
    @Override
    public List<Permission> findByPermissionTypeInAndTargetId(Set<String> permissionTypes, String targetId) {
        return permissionRepository.findByPermissionTypeInAndTargetId(permissionTypes, targetId);
    }
