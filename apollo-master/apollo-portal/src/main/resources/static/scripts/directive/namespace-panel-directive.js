            ...
            function initNamespace(namespace, viewType) {
                ...
                function initPermission(namespace) {
                    ...
                    PermissionService.has_modify_namespace_permission(
                        ...
                        namespace.baseInfo.namespaceName)
                        .then(function (result) {
                            var hasAddDelPermission = result[0].hasPermission;
                            var hasModifyValPermission = result[1].hasPermission;

                            //branch has same permission
                            namespace.hasModifyPermission = hasAddDelPermission && hasModifyValPermission;
                            if (namespace.branch) {
                                namespace.branch.hasModifyPermission = namespace.hasModifyPermission;
                            }
                        });
                    ...
                    PermissionService.has_add_or_del_namespace_permission(
                        scope.appId,
                        namespace.baseInfo.namespaceName)
                        .then(function (result) {
                            //branch has same permission
                            namespace.hasAddDelPermission = result.hasPermission;
                            if (namespace.branch) {
                                namespace.branch.hasAddDelPermission = result.hasPermission;
                            }
                        });

                    PermissionService.has_modify_val_namespace_permission(
                        scope.appId,
                        namespace.baseInfo.namespaceName)
                        .then(function (result) {
                            //branch has same permission
                            namespace.hasModifyValPermission = result.hasPermission;
                            if (namespace.branch) {
                                namespace.branch.hasModifyValPermission = result.hasPermission;
                            }
                        });
