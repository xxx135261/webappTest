...
function SettingController($scope, $location, toastr,
    ...
    $scope.submitBtn2Disabled = false;
    $scope.userSelectWidgetId = 'toAssignMasterRoleUser';
    $scope.implUserSelectWidgetId = 'toAssignImplRoleUser';

    $scope.assignMasterRoleToUser = assignMasterRoleToUser;
    $scope.removeMasterRoleFromUser = removeMasterRoleFromUser;
    $scope.assignImplRoleToUser = assignImplRoleToUser;
    $scope.removeImplRoleFromUser = removeImplRoleFromUser;
     ...
     * 初始化运维人员列表
     */
    function initImplementations() {
        PermissionService.get_app_impl_role_users($scope.pageContext.appId)
            .then(function (result) {
                $scope.implUsers = result.masterUsers;
            });
    }
    ...
    function assignImplRoleToUser() {
        var user = $('.' + $scope.implUserSelectWidgetId).select2('data')[0];
        if (!user) {
            toastr.warning("请选择用户");
            return;
        }
        var toAssignImplRoleUser = user.id;
        $scope.submitBtn2Disabled = true;
        PermissionService.assign_impl_role($scope.pageContext.appId,
            toAssignImplRoleUser)
            .then(function (result) {
                $scope.submitBtn2Disabled = false;
                toastr.success("添加成功");
                $scope.implUsers.push({userId: toAssignImplRoleUser});
                $('.' + $scope.implUserSelectWidgetId).select2("val", "");
            }, function (result) {
                $scope.submitBtn2Disabled = false;
                toastr.error(AppUtil.errorMsg(result), "添加失败");
            });
    ...
    function removeImplRoleFromUser(user) {
        PermissionService.remove_impl_role($scope.pageContext.appId, user)
            .then(function (result) {
                toastr.success("删除成功");
                removeUserFromList($scope.implUsers, user);
            }, function (result) {
