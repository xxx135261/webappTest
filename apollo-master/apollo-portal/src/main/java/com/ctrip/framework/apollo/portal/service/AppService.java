...
public class AppService {
   ...
   * 自增APPID起始值
   */
  private static final long AUTO_INCREMENT_APPID_START = 100000000;
  ...
  public String generateAppId() {
    Long maxId = appRepository.getMaxId();
    Long genId = maxId != null ? (maxId + AUTO_INCREMENT_APPID_START) : AUTO_INCREMENT_APPID_START;
    App app = appRepository.findByAppId(genId.toString());
    if (app != null) {
      boolean isGenSuccess = false;
      // 如果自增APPID已经存在，则加1000继续尝试
      while (!isGenSuccess) {
        genId += 1000;
        app = appRepository.findByAppId(genId.toString());
        if (app == null) {
          isGenSuccess = true;
        }
      }
