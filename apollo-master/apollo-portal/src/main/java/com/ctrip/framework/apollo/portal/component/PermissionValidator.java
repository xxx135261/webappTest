...
public class PermissionValidator {
  ...
  public boolean hasModifyNamespacePermission(String appId, String namespaceName) {
    return rolePermissionService.userHasPermission(userInfoHolder.getUser().getUserId(),
        ...
        || (hasAddDelNamespacePermission(appId, namespaceName) && hasModifyValNamespacePermission(appId, namespaceName));
  }

  public boolean hasModifyValNamespacePermission(String appId, String namespaceName) {
    return rolePermissionService.userHasPermission(userInfoHolder.getUser().getUserId(),
        PermissionType.MODIFY_VAL_NAMESPACE,
        RoleUtils.buildNamespaceTargetId(appId, namespaceName));
  }

  public boolean hasAddDelNamespacePermission(String appId, String namespaceName) {
    return rolePermissionService.userHasPermission(userInfoHolder.getUser().getUserId(),
        PermissionType.ADD_DEL_NAMESPACE,
        RoleUtils.buildNamespaceTargetId(appId, namespaceName));
  }
