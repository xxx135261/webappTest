...
CREATE TABLE Instance (
  ...
  PRIMARY KEY (Id),
  UNIQUE IX_INSTANCE (AppId,ClusterName,Ip,DataCenter)
) COMMENT='使用配置的应用实例';
...
CREATE TABLE InstanceConfig (
  ...
  DataChange_LastTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最后修改时间',
  PRIMARY KEY (Id),
  UNIQUE IX_INSTANCECONFIG (InstanceId,ConfigAppId,ConfigNamespaceName)
) COMMENT='应用实例的配置信息';
...
CREATE TABLE NamespaceLock (
  ...
  DataChange_LastTime timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最后修改时间',
  IsDeleted bit(1) DEFAULT 0 COMMENT '软删除',
  PRIMARY KEY (Id),
  UNIQUE IX_NamespaceId (NamespaceId)
...
CREATE TABLE Release (
  ...
  IsAbandoned bit(1) NOT NULL DEFAULT 0 COMMENT '是否废弃',
  IsDeleted bit(1) NOT NULL DEFAULT 0 COMMENT '1: deleted, 0: normal',
  DataChange_CreatedBy varchar(32) NOT NULL DEFAULT 'default' COMMENT '创建人邮箱前缀',
  DataChange_CreatedTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  DataChange_LastModifiedBy varchar(32) DEFAULT '' COMMENT '最后修改人邮箱前缀',
